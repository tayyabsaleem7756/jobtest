...
import { Application, KYCState } from '../../interfaces/KnowYourCustomer'
...
export const kycSlice = createSlice({
	...
	reducers: {
		...
		setKYCAnswers: (
			state,
			action: PayloadAction<WorkflowAnswerPayload>,
		) => {
		...
		setWorkflow: (
			state,
			{ payload }: PayloadAction<WorkFlow | undefined>,
		) => {
			state.workflow = payload
		},
		setApplicationStatus: (
			state,
			{ payload }: PayloadAction<WorkFlowStatus | null>,
		) => {
	...
	extraReducers: builder => {
		builder.addCase(fetchKYCRecord.fulfilled, (state, actions) => {
			...
			state.workflow = getAMLWorkFlowByTypeSlug(
				state.fundWorkflows,
				workflowSlug,
			)
			if (participants === null) {
				...
				participants.forEach(participantRecord => {
					...
					const { answers } = parseKYCRecord(
						participantRecord,
						state.kycParticipantsDocuments[participantRecord.id],
					)
		...
		builder.addCase(fetchKYCDocuments.fulfilled, (state, actions) => {
		...
		builder.addCase(
			fetchCommentsByKycRecordId.fulfilled,
			(state, { payload }) => {
				state.didFetch.comments = true
				const comments = payload as Comment[]
				state.comments = uniqBy(
					[...comments, ...cloneDeep(state.comments)],
					comment => comment.id,
				)
				state.commentsByRecord = mergeComments(
					state.commentsByRecord,
					comments,
				)
			},
		)
		builder.addCase(fetchCommentsByKycRecordId.pending, state => {
			state.didFetch.comments = false
		})
		builder.addCase(
			fetchCommentsByApplicationId.fulfilled,
			(state, { payload }) => {
				state.didFetch.comments = true
				const comments = payload as Comment[]
				state.comments = uniqBy(
		...
		builder.addCase(fetchCommentsByApplicationId.pending, state => {
			state.didFetch.comments = false
		})
		builder.addCase(fetchWorkflows.fulfilled, (state, { payload }) => {
			...
			state.workflow = getAMLWorkFlowByTypeSlug(
				fundWorkflows,
				state.selectedWorkflowSlug,
			)
		})
		builder.addCase(
			fetchAdditionalCards.fulfilled,
			(state, { payload }) => {
		...
		builder.addCase(fetchInitialRecord.fulfilled, (state, { payload }) => {
			...
			if (kyc_record) {
				...
				if (participants === null) {
					...
					participants.forEach(participantRecord => {
						...
						const { answers } = parseKYCRecord(
							participantRecord,
							state.kycParticipantsDocuments[
								participantRecord.id
							],
						)
						// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
						state.kycRecordParticipants![participantRecord.id] =
							answers
					})
				}
				state.didFetch.kycRecord = true
		...
		builder.addCase(
			fetchKYCParticipantsDocuments.fulfilled,
			(state, { payload }) => {
				if (!payload) return
				const { participantId, documents } = payload
				state.didFetch.participantDocuments = true
				state.kycParticipantsDocuments[participantId] =
					documents as KYCDocument[]
				const docAnswers = parseDocumentsToAnswers(documents)
				if (state.kycRecordParticipants !== null) {
					// TODO : reconsider this
					// eslint-disable-next-line prefer-object-spread
					state.kycRecordParticipants[participantId] = Object.assign(
						{},
						state.kycRecordParticipants[participantId],
						docAnswers,
					)
				}
			},
		...
		builder.addCase(
			createFirstParticipant.fulfilled,
			(state, { payload }) => {
				const { id } = payload
				const { answers } = parseKYCRecord(
					payload,
					state.kycParticipantsDocuments[id],
				)
				// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		...
		builder.addCase(createFirstParticipant.pending, state => {
		...
		builder.addCase(fetchKYCRecord.rejected, state => {
			state.didFetch.kycRecord = true
			state.answers = {}
		})
		builder.addCase(createFirstParticipant.rejected, state => {
		...
		builder.addCase(
			fetchApplicationDocumentRequests.fulfilled,
			(state, { payload }) => {
				state.applicationDocumentsRequests = payload
			},
		)
		builder.addCase(
			fetchApplicationDocumentRequestResponse.fulfilled,
			(state, { payload }) => {
				state.applicationRequestedDocuments = payload
			},
		)
		builder.addCase(
			getFundCriteriaResponseStatus.fulfilled,
			(state, { payload }) => {
