...
const CapitalCallsDetail = ({
  ...
  const { due_date, can_approve, document, task_id, approved_at, are_documents_generated } = capitalCall;

  const hasCompleteDocs = (arr: any[], field: string) => {
    return arr.length === arr.filter((item) => item[field] !== null).length;
  };
  ...
  const generateFooterData = (data: any[]) => {
    return {
      ...
      amount: data.reduce((total, obj) => total + (obj.amount || 0), 0),
      investment: data.reduce((total, obj) => total + (obj.investment || 0), 0),
      ...
      amount_received: data.reduce(
        (total, obj) => total + (obj.amount_received || 0),
        0
      ),
    };
  };
  ...
  return (
    <div>
      ...
      <Row>
        <RowItem>
          <Title>Capital Call Amount</Title>
          <AmountCell>
            <FormattedCurrency
        ...
        <RowItem>
          <Text>Capital Call Date: </Text>
          <Text style={{ color: "#607D8B" }}>{standardizeDate(due_date)}</Text>
        </RowItem>
      </Row>
      <RsuiteTable
        isLoading={false}
