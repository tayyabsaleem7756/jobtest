...
import { getFundCriteriaResponseStatus } from "../EligibilityCriteria/thunks";
...
const ApplicationView: FunctionComponent<any> = () => {
  ...
  const applicationStatusApproved = applicationStatus?.is_approved;
  ...
  const onChangeAMLKYCAnswers = async () => {
    ...
    history.push(`/${INVESTOR_URL_PREFIX}/funds/${externalId}/amlkyc`);
  };
  ...
  // console.log({applicationNextState})

  const getCommentCountData = () => {
    const applicationCountData = application.reduce((acc: any, card: any) => {
      acc[card.name] = getCommentsCount(card);
      return acc;
    }, {});

    const generateParticipantsLabel = (participantId: any, index: number) => {
      let label = "";
      if (kycRecordParticipants) {
        const participant = kycRecordParticipants[participantId];
        label = participant
    ...
    const participentsLabelCount: Record<string, any> = {};
    Object.keys(participantCommentsCount).forEach((id, index) => {
      const label = generateParticipantsLabel(id, index);
      participentsLabelCount[label] = participantCommentsCount[id];
    });

    const countData = {
      "Investor Information": overviewCommentsCount,
      ...(investmentCard
        ? {
            [investmentCard.name || "Invesment Amount"]:
              getCommentsCount(investmentCard),
            [eligibilityCriteriaCard?.name || "Eligibility Criteria"]:
              getCommentsCount(eligibilityCriteriaCard),
          }
        : {}),
      ...applicationCountData,
      "Tax Forms": taxCommentsCount,
      "Banking Details": bankingCommentsCount,
      "Fund Documents":
        fundDocumentCommentsCount + additionalDocumentPendingRequestsCount,
      ...participentsLabelCount,
      POA: programCommentsCount,
    };
    ...
    const sequence = [
      ...
      "Corporate Entity",
      "Corporate Documents",
      ...Object.keys(participentsLabelCount),
      "Tax Forms",
      "Banking Details",
      "Fund Documents",
      "POA",
    ];
    return { countData, sequence };
  };
            ...
            <LeftSidebar>
             ...
             <Content>
               <FormContainer>
                 ...
                 <NextStatus applicationNextState={applicationNextState} getCommentCountData={getCommentCountData}/>
                ...
                </CardContainer>}
                {
                  ...
                  <ScrollElement name={getSectionId(eligibilityCriteriaCard.name)}>
                    ...
                    onClick={() => {
                      ...
                      history.push(
                        `/${INVESTOR_URL_PREFIX}/funds/${externalId}/onboarding${MODIFY_ELIGBILITY}`
                      );
                    }}
                  >
