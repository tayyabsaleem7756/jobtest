from api.cards.default.individual_cards import US_PERSON_FIELDS, POST_NET_WORTH_ENTITY_FIELDS
...
from api.cards.default import individual_cards, private_company_cards, limited_partnership_cards, trust_cards
...
class DefaultCards:
    ...
    def _get_personal_card():
        ...
        uk_country_option = next(item for item in country_options if item["label"] == "United Kingdom")
        personal_card = copy.deepcopy(individual_cards.PERSONAL_CARD)
        for item in personal_card['schema']:
            if item['id'] == 'uk_national_insurance_number':
                item['field_dependencies'][0]['value'] = uk_country_option['value']
            if item['id'] in ['citizenship_country', 'office_location']:
    ...
    @staticmethod
    def _add_uk_country_value(schema):
        UK_Country = Country.objects.get(iso_code='UK')
        personal_card = copy.deepcopy(schema)
        for item in personal_card['schema']:
            if item['id'] == 'uk_national_insurance_number':
                item['field_dependencies'][0]['value'] = UK_Country.id
        return personal_card
    ...
    def _get_private_company_personal_cards():
        ...
        personal_card = DefaultCards._add_uk_country_value(personal_card)
